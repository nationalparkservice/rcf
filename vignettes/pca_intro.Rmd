...
```{r}
pca_means <- cf_pca("BAND_test", data = pca_summary, variables = "all_threshold", directory = my_directory)
```
...
## Summarize thresholds using PCA models
...
```{r}
threshold_summary <- pca_thresholds("BAND_test", pca_data = pca_means, all_data = thresholds, summarize_by = "year", directory = my_directory)
```
...
```{r}
threshold_summary$time <- factor(threshold_summary$time, levels = c("Historical", "Future"))
