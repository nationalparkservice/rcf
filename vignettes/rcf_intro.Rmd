...
having access to this data and tools to work with it supports location-specific planning efforts that
incorporate climate change.

The Global Circulation Models (GCM's) used within this package are those 

The methods contained herein aggregate GCM's into four general groupings representative of plausible and divergent climate futures:
  (1) Warm & Wet
  (2) Warm & Dry (or Damp)*
  (3) Hot & Wet
  (4) Hot & Dry (or Damp)*
  
Then, the user may select any of three methods 

# Package goals

1. Automatically select Global Circulation Models (GCM's) 


## About this tutorial

This vignette is the first in a series of two, and is intended for general users of the `rcf` package. Advanced users interested in Principal Component Analyses (PCA) may continue on with [the second package in the series]() 

The functions in this package allow users to automatically select Global Circulation Models (GCMs) that best represent four climate futures (CFs): (1) Warm Wet, (2) Warm Dry (or Damp), (3) Hot Wet, and (4) Hot Dry (or Damp)*.

This package can also be used to produce summaries of threshold exceedances for 25 metrics using one of three methods (quadrant, corner, or PCA) and one of three durations (month, season, or year).
...
This package expects the user to have a basic understanding of GCMs, each of which represents a plausible climate future. The GCMs in this package are based off of the downscaled climate model [MACA](http://www.climatologylab.org/maca.html) (Multivariate Adaptive Climate Analog) Version 2. Additionally, this vignette assumes familiarity with routine functions in the `tidyverse`, specifically `readr` and the `read_csv()` function; `filter()`, `mutate()` and `select()` in the package `dplyr`; and basic syntax for plot creation in `ggplot2`.
...
The files in this package, especially the raw data, are large. If the user is downloading data in this package using `rcf_data()`, at least *500 MB* of space must be available. If data have been obtained from elsewhere, specifically for the `cf_pca()` function, the user must ensure that variable names in the `rcf` package match the column names of the outside dataset exactly.

This vignette follows [Best practices](https://rstats.wtf/project-oriented-workflow.html#work-in-a-project) in R

[Best practices](https://rstats.wtf/project-oriented-workflow.html#work-in-a-project) in R use project directories, so this vignette will walk through the steps as if this project is using a .Rproj file inside of a directory that will hold all information and resultant data for this project. This vignette will also use the `here()` package, rather than relative file paths.
...
For most users of this package, there will only be one more function to run before moving on to graphing. Advanced users can continue to follow along with this vignette or choose to move onto the [PCA intro](INSERT LINK) vignette.
...
ggplot(data = corner_month_hw, aes(x = month, y = precip_monthly)) +
  ...
  theme(text = element_text(size = 20),
        ...
        plot.title = element_text(hjust = 0.5)) +
        #axis.text.x = element_blank()) +
  labs(y = "Monthly Precipitation (in)", x = "Month",
